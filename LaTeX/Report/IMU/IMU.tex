...
For steady-level flight it can be assumed that angular velocities are the same in body and NED reference frames \cite{AircraftDynamics} so we do not need to perform rotational transformation on these values. Therefore the angular velocity output can  be used($\dot{\Phi}, \dot{\Theta}, \dot{\Psi}$,) from the drone plant as the input for the gyroscope plant. 
