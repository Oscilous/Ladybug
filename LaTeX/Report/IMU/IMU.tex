...
\begin{figure}[H]
    ...
    \end{center}
    \caption{NED coordinate system \site{Coordinate_sys}}
    \label{fig:my_label}
\end{figure}
...
Any object rotation can be described by multiplying a rotational matrix with the original position vector. Any rotation matrix R can be decomposed as a product of three elemental rotation matrices - Roll, Yaw and Pitch.\site{Rotation}\newline
\begin{figure}[H]
    \begin{center}
...
Since the filter will be digital, the real time transfer function needs to be discretized by moving to the Z domain. This can be done with the help of Bilinear transform (Tustin's method), with the following substitution (where $T_s$ is the sampling time)\site{Digital_filters}:
\begin{displaymath}
...
After the calculated angles are passed through their respective filters, the values can be summed up resulting in a complimentary filter (as shown in \ref{fig:compl_filt}):
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Complimentary filter implementation in discrete time \site{Complementary_Filters}}
...
For steady-level flight it can be assumed that angular velocities are the same in body and NED reference frames \site{AircraftDynamics} so we do not need to perform rotational transformation on these values. Therefore the angular velocity output can  be used($\dot{\Phi}, \dot{\Theta}, \dot{\Psi}$,) from the drone plant as the input for the gyroscope plant. 
...
The LSM6DS3.h \sites{LSM6DS3} pre-defines the I2C address as 0x6B but can be overwritten in code - as in our case the address is 0x6A. 
...
For the time of flight sensor it is needed to include the $Adafruit_VL53L0X.h$ library \site{VL53L0XV2} which then allows the vertical distance to be measured with the help of measure.RangeMilliMeter function.
...
This can be done with the help of rotational matrix as follows in \ref{fig:calc_z}:
...
To choose optimal cut-off frequencies for the digital filters, the Complimentary filter output can be printed on Serial Plotter alongside the unfiltered Euler angles from gyroscope and accelerometer to see how much the filtered signal is attenuated and observe the phase shift. It can be seen in \ref{fig:serial_plot} that the gyroscope calculation has drifted in time and the Complimentary filter output has smoothed out some of the acclerometer readings noise, however there is a slight delay in time between the signals.
